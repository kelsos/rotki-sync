...
func fakeRoot() *cobra.Command {
	c := &cobra.Command{Use: "rotki-sync"}
	c.AddCommand(&cobra.Command{Use: "version", Run: func(*cobra.Command, []string) {}})
	return c
}
...
func TestInstallFishCompletion(t *testing.T) {
	...
	path := filepath.Join(home, ".config", "fish", "completions", "rotki-sync.fish")
...
func TestInstallBashCompletion(t *testing.T) {
	...
	path := filepath.Join(home, ".local", "share", "bash-completion", "completions", "rotki-sync")
	assertNonEmptyFile(t, path)
}
...
func TestInstallZshCompletionFallback(t *testing.T) {
	...
	assertNonEmptyFile(t, filepath.Join(home, ".zsh", "completions", "_rotki-sync"))
}
...
func TestInstallIsIdempotentUpdate(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	path := filepath.Join(home, ".zsh", "completions", "_rotki-sync")
	if err := installZshCompletion(fakeRoot()); err != nil {
		t.Fatal(err)
	}
